 ...
 *
 *   1. Is the vp-affiliates plugin on cms.vertalispeptides.com actually
...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  ...
  const results = await Promise.all([
    ...
    probe('materials-GET', `${WC_URL}/wp-json/vp-affiliates/v1/materials?storefront=vertalis`),
    ...
    probe('auth-login-POST-plain', `${WC_URL}/wp-json/vp-affiliates/v1/auth/login`, {
      method: 'POST',
      headers: json,
      body: JSON.stringify({ email: 'diag-probe@example.com', secret: 'diag-probe-not-real', storefront: 'vertalis' }),
    }),
    probe('auth-login-POST-browser-headers', `${WC_URL}/wp-json/vp-affiliates/v1/auth/login`, {
      ...
      headers: {
        ...
        'Origin': 'https://affiliate.vertalispeptides.com',
        'Referer': 'https://affiliate.vertalispeptides.com/',
      },
      body: JSON.stringify({ email: 'diag-probe@example.com', secret: 'diag-probe-not-real', storefront: 'vertalis' }),
    }),
